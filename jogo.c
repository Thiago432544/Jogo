...
Mix_Chunk* somdetiro = NULL;
Mix_Chunk* somdegolpe = NULL;
Mix_Music* somdefundo = NULL;
Mix_Chunk* somdemorte = NULL;
...
int initialize_window(void) {
    ...
    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0) {
        fprintf(stderr, "Erro ao iniciar SDL_MIXER: %s\n", Mix_GetError());
        return FALSE;
    }
...
void fire_projectile(int from_square) {
    ...
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        if (!projectiles[i].is_active) {
            ...
            Mix_PlayChannel(-1, somdetiro, 0); // som do tiro (-1 para o canal, somdetiro sendo a chunk e 0 sendo a quantidade de loop)
            ...
            if (strcmp(personagem, "python") == 0) {
                tmpSurface = IMG_Load("assets/python.png");  // Projétil especial para Python
            }
            else {
...
void efeitos_sonoros() {

    //testetiro (algmmematapeloamordedeus)

    somdetiro = Mix_LoadWAV("assets/EfeitoTiro1.wav");
    if (!somdetiro) {
        fprintf(stderr, "Erro no momento do peteco-peteco: %s\n", Mix_GetError());
        return;
    }

    //testebackground (easteregg?)

    somdefundo = Mix_LoadMUS("assets/Background.ogg");
    if (!somdefundo) {
        fprintf(stderr, "Erro ao carregar o rock pesado: %s\n", Mix_GetError());
        return;
    }

    //testedanorecebido (ainnn)

    somdegolpe = Mix_LoadWAV("assets/Danorecebido.wav");
    if (!somdegolpe) {
        fprintf(stderr, "Erro ao carregar a lapada: %s\n", Mix_GetError());
        return;
    }

    //testemorreu (morreu de morte morrida)

    somdemorte = Mix_LoadWAV("assets/Efeitomorte1.wav");
    if (!somdemorte) {
        fprintf(stderr, "Erro ao carregar a morte: %s\n", Mix_GetError());
        return;
    }

    //volume (temp, mover para constantes.h)

    Mix_VolumeChunk(somdetiro, 24);  // 128 = 100% volume
    Mix_VolumeMusic(10);             //  64 =  50% volume (Música de fundo paizão, tenha calma)
    Mix_VolumeChunk(somdegolpe, 24); //  32 =  25% volume
    Mix_VolumeChunk(somdemorte, 40);

    Mix_PlayMusic(somdefundo, -1);   // Loop para iniciar a música (caso necessário modificar por aqui para que a música toque de acordo com o mapa escolhido).
    ...
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        if (projectiles[i].is_active) {
            if (!projectiles[i].from_square &&
                ...
                Mix_PlayChannel(-1, somdegolpe, 0); // som de dano;
            ...
            if (ball_life == 0) {
                ...
                Mix_PlayChannel(-1, somdemorte, 0); // som de morte
                game_is_running = FALSE;
            }
            if (square_life == 0) {
...
void setup() {
    ...
    num_obstaculos[1] = 8;
    obstaculos[1][0] = (obstacle){ 550, 250, 25, 150 };     // proteção de cima do ball
    obstaculos[1][1] = (obstacle){ 550, 700, 25, 150 };     // proteção de baixo do ball
    obstaculos[1][2] = (obstacle){ 750, 475, 25, 150 };     // proteção do meio do ball

    obstaculos[1][3] = (obstacle){ 1370, 250, 25, 150 };     // proteção de cima do square
    obstaculos[1][4] = (obstacle){ 1370, 700, 25, 150 };     // proteção do meio do square
    obstaculos[1][5] = (obstacle){ 1170, 475, 25, 150 };     // proteção de baixo do square

    obstaculos[1][6] = (obstacle){ 865, 0, 200, 200 };       // caixa de cima
    obstaculos[1][7] = (obstacle){ 865, 885, 200, 200 };     // caixa de baixo
    ...
    if (tmpSurface) {
        ...
        // Carrega uma textura padrão se necessário
        tmpSurface = IMG_Load("assets/tale.png");  // Mesma fallback
        if (tmpSurface) {
            square.texture = SDL_CreateTextureFromSurface(renderer, tmpSurface);
            SDL_FreeSurface(tmpSurface);
        }
