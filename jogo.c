...
void setup() {
   ...
   // obstaculos[1][2] = (obstacle){ 350, 450, 120, 60 };
    ...
    ball.width = 75;
    ball.height = 75;
    ball.last_dir = DIR_RIGHT; // Dire��o inicial padr�o
...
void update() {
    ...
    // Movimento da bola (ball) com checagem de obst�culos
    ...
    // Verifica colis�o da bola com obst�culos
    ...
    // Movimento do quadrado (square) com checagem de obst�culos
    ...
    // Verifica colis�o do quadrado com obst�culos
    ...
    if (!square_collision) {
        ...
        for (int i = 0; i < MAX_PROJECTILES; i++) {
            if (projectiles[i].is_active) {
                ...
                if (obstacle_collision) {
                    projectiles[i].is_active = FALSE;
                    continue; // Pula para o pr�ximo proj�til
                }
            }
        }
        ...
        if (check_collision(ball.x, ball.y, ball.width, ball.height,
            square.x, square.y, square.width, square.height)) {
            printf("COLIS�O DETECTADA ENTRE BALL E SQUARE!\n");
        }

        // Limitar posi��o de ball
